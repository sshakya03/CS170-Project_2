...
def forward_selection(data):
    print("Running updated forward_selection function")  # Debugging line
    current_set_of_features = []
    best_accuracy = 0
    best_set_of_features = []
    for i in range(1, len(data[0])):
        feature_to_add_on_level = []
        best_accuracy_on_level = 0
        for k in range(1, len(data[0])):
            if k not in current_set_of_features:
                features_to_test = [k] + current_set_of_features
                accuracy = leave_one_out_cross_validation(data,features_to_test,k+1)
...
def backward_elimination(data):
    ...
    best_accuracy = leave_one_out_cross_validation(data,current_set_of_features,0) # 0 is placeholder
    print(f"Feature set {{{', '.join(map(str, current_set_of_features))}}} was best, accuracy is {best_accuracy:.1f}%")
    best_set_of_features = current_set_of_features.copy()
    for i in range(1, len(data[0])-1):
        feature_to_remove_on_level = []
        best_accuracy_on_level = 0
        for k in range(1, len(data[0])):
